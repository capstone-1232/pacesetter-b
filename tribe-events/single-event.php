	...
	<?php while ( have_posts() ) :  the_post(); ?>
            <div class="flex">
                <div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
                    ...
                    <?php $featured = tribe_event_featured_image( $event_id, 'full', false ); 
                    echo $featured;?>
                <!-- Event content -->
...
<?php
global $post;
// Assuming $event_id is the ID of the current event
$event_tags = get_the_tags( $event_id );

if ( ! empty( $event_tags ) ) {
    // Initialize an array to store tax_query clauses
	$tax_queries = array();
	...
	$args = array(
		...
		'tag__in' => wp_get_post_tags($event_id, array('fields' => 'ids')),
    ...
    if ( $related_events_query->have_posts() ) {
        while ( $related_events_query->have_posts() ) {
            ...
            $related_event_content = get_the_content();
			$related_event_image_url = get_the_post_thumbnail_url();
			$related_post_url = get_permalink( get_the_ID() );

            // Your code for each related event goes here
			?>
				<div class="related-events__card">
					<a href="<?php echo $related_post_url?>">
					<img src="<?php echo $related_event_image_url;?>" alt="">
				</a>	
