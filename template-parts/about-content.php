<section class="front-page-about">
    <h2>About Pacesetter</h2>
    <div class="about-flex">
        <div class="about-content">
            <p style="background-image: radial-gradient(circle, rgba(0,0,0,0.90) 0%, rgba(52,52,52,0.30) 100%), url('<?php echo home_url(); ?>/wp-content/themes/pacesetter-b/img/inside-store.jpg');">Pacesetter Ski and Snowboard Store is your premier destination for all things winter sports. With a curated
                selection of top-quality equipment and gear, combined with expert knowledge and personalized customer service,
                we strive to elevate your skiing and snowboarding experience to new heights. Whether you're a seasoned pro or
                hitting the slopes for the first time, trust Pacesetter to outfit you for adventure in style and comfort.</p>
            <div class="links">
                <a class="orange-btn" href="/about-us">Read More</a>
                <a class="white-btn" href="/contact-us">Contact Us</a>
            </div>
        </div>
        <div class="about-image" style="background-image: url('<?php echo home_url(); ?>/wp-content/themes/pacesetter-b/img/inside-store.jpg');"></div>
    </div>
</section>